...
class ErrorCard:
    ...
    def dismiss_error_card(self):
        ...
        if self.error_frame:
            ...
            AnimationManager.fade_out(
                self.error_frame, duration=200, callback=self._destroy_error_frame
            )
    ...
    def _get_level_styling(self, level: ErrorLevel) -> tuple:
        ...
        styling = {
            ErrorLevel.INFO: (self.theme_colors["info_bg"], self.theme_colors["info_border"], "ℹ️"),
            ErrorLevel.WARNING: (
        ...
        def dismiss_toast():
            if toast_frame in self.active_toasts:
                ...
                AnimationManager.slide_in(
                    toast_frame, direction="right", duration=300, distance=100
                )
...
class ErrorManager:
    ...
    def show_loading_error(self, operation: str, error_details: str):
        ...
        self.show_error(
            ...
            actions=[
                {"text": "Retry", "callback": lambda: self._retry_operation(operation)},
                {"text": "Cancel", "callback": self.error_card.dismiss_error_card},
            ],
        )
    ...
    def show_network_error(self, details: str = ""):
        ...
        self.show_error(
            ...
            actions=[{"text": "Retry", "callback": lambda: self._retry_network_operation()}],
        )
    ...
    def show_api_error(self, api_name: str, status_code: int, details: str = ""):
        ...
        self.show_error(
            ...
            actions=[{"text": "Retry", "callback": lambda: self._retry_api_call(api_name)}],
        )
