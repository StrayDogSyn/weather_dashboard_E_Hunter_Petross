...
class StatusMessageManager:
    ...
    def set_loading_state(
        ...
        message = contextual_message or self.contextual_help.get(
            f"loading_{operation.lower()}", f"{operation}..."
        )
    ...
    def set_success_state(self, message: str, auto_clear: int = 3000):
        """Set success state."""
        self.set_status(message=message, status_type=StatusType.SUCCESS, auto_clear=auto_clear)

    def set_error_state(self, message: str, auto_clear: int = 5000):
        """Set error state."""
        self.set_status(message=message, status_type=StatusType.ERROR, auto_clear=auto_clear)
    ...
    def set_ready_state(self):
        ...
        if random.random() < 0.7:  # 70% chance for tip
            ...
            self.set_status(message=tip, status_type=StatusType.TIP, auto_clear=8000)
        else:
            self.set_status(
                message="Ready - Weather dashboard is up to date", status_type=StatusType.READY
            )
    ...
    def show_contextual_help(self, context: str):
        ...
        help_message = self.contextual_help.get(
            context, "Need help? Check the documentation for more information."
        )

        self.set_status(message=help_message, status_type=StatusType.INFO, auto_clear=5000)
        ...
        def type_char(index: int = 0):
            try:
                ...
                if index <= len(message):
                    ...
                    if index < len(message):
                        self.typing_timer = self.safe_after(delay, lambda: type_char(index + 1))
            except tk.TclError:
                # Widget has been destroyed
                return
...
class TooltipManager:
    ...
    def add_tooltip(
        self, widget: ctk.CTkBaseClass, text: str, delay: int = 500, follow_mouse: bool = True
    ):
            ...
            if alpha < 0.9:
                ...
                try:
                    tooltip_window.after(
                        20, lambda: self._fade_in_tooltip(tooltip_window, alpha + 0.1)
                    )
                except tk.TclError:
                    pass
            else:
