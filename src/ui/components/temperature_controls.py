...
class TemperatureControls(GlassmorphicFrame):
    ...
    def __init__(self, parent, initial_unit: TemperatureUnit = TemperatureUnit.FAHRENHEIT,
                 on_unit_change: Optional[Callable[[TemperatureUnit], None]] = None,
                 show_feels_like: bool = True, show_heat_index: bool = True,
                 **kwargs):
    ...
    def _setup_ui(self) -> None:
        ...
        main_frame.grid(row=0, column=0, sticky="ew", padx=10, pady=5)
    ...
    def _create_unit_selector(self, parent) -> None:
        ...
        unit_frame.grid(row=0, column=0, sticky="ew", padx=(0, 10), pady=5)
        ...
        unit_label.grid(row=0, column=0, columnspan=3, sticky="w", padx=10, pady=(10, 5))
        ...
        for i, unit in enumerate(TemperatureUnit):
            ...
            button.grid(row=1, column=i, padx=5, pady=(0, 10), sticky="ew")
    ...
    def _create_temperature_display(self, parent) -> None:
        ...
        display_frame.grid(row=0, column=1, sticky="ew", padx=(0, 10), pady=5)
        ...
        if self.show_feels_like:
            feels_like_frame = tk.Frame(secondary_frame, bg=self.style.colors['surface'])
        ...
        if self.show_heat_index:
            heat_index_frame = tk.Frame(secondary_frame, bg=self.style.colors['surface'])
    ...
    def _create_additional_controls(self, parent) -> None:
        ...
        controls_frame.grid(row=0, column=2, sticky="ew", pady=5)
    ...
    def _change_unit(self, new_unit: TemperatureUnit) -> None:
        ...
        if self.current_temp is not None:
            self.current_temp = self._convert_temperature(self.current_temp, old_unit, new_unit)
        if self.feels_like_temp is not None:
            self.feels_like_temp = self._convert_temperature(self.feels_like_temp, old_unit, new_unit)
        if self.heat_index is not None:
            self.heat_index = self._convert_temperature(self.heat_index, old_unit, new_unit)
        if self.wind_chill is not None:
            self.wind_chill = self._convert_temperature(self.wind_chill, old_unit, new_unit)
    ...
    def set_temperature(self, temp: float, feels_like: Optional[float] = None,
                       heat_index: Optional[float] = None, 
                       wind_chill: Optional[float] = None) -> None:
