...
class ModernButton(tk.Button):
    ...
    def _apply_hover_effect(self) -> None:
        ...
        self.config(bg=hover_bg, relief="raised", borderwidth=1)

    def _remove_hover_effect(self) -> None:
        """Remove hover visual effects."""
        self.config(bg=self.original_bg, relief="flat", borderwidth=0)
    ...
    def set_enabled(self, enabled: bool) -> None:
        ...
        if enabled:
            self.config(
                state="normal", cursor="hand2", bg=self.original_bg, fg=self.original_fg
            )
        else:
            self.config(
                state="disabled",
    ...
    def set_loading(self, loading: bool, loading_text: str = "Loading...") -> None:
        ...
        if loading:
            ...
            self.config(text=loading_text, state="disabled", cursor="wait")
        else:
            if hasattr(self, "original_text"):
                self.config(text=self.original_text, state="normal", cursor="hand2")
