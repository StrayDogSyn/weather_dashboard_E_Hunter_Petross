...
from ..services.model_integration_service import ModelIntegrationService, IntegratedForecast
...
class ForecastVisualizationFrame(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        self.city_entry = ttk.Entry(self.options_frame, textvariable=self.city_var, width=20)
        ...
        self.refresh_btn = ttk.Button(
            self.options_frame,
            text="Refresh",
            command=self._refresh_forecast
        )
    ...
    def _generate_forecast(self):
        ...
        def run_async():
            try:
                ...
                self.after(0, lambda: self.status_var.set(f"Forecast generated for {city} - Confidence: {forecast.confidence_score:.1%}"))
                
            except Exception as e:
                self.logger.error(f"Error generating forecast: {e}")
                self.after(0, lambda: messagebox.showerror("Error", f"Failed to generate forecast: {str(e)}"))
    ...
    def _refresh_forecast(self):
        ...
        if self.current_city:
            ...
            messagebox.showinfo("Info", "No forecast to refresh. Generate a forecast first.")
    ...
    def _update_temperature_chart(self):
        ...
        if api_temps and len(api_temps) == len(days):
            ax.plot(days, api_temps, 'o-', label='API Forecast', color='blue', linewidth=2)
        
        # Plot ML predictions
        if ml_temps:
            ax.plot(days, ml_temps, 's-', label='ML Prediction', color='red', linewidth=2)
            
            # Add confidence intervals
            if confidence_intervals:
                lower_bounds = [ci[0] for ci in confidence_intervals]
                upper_bounds = [ci[1] for ci in confidence_intervals]
                ax.fill_between(days, lower_bounds, upper_bounds, alpha=0.3, color='red', label='Confidence Interval')
        ...
        if hybrid_temps and len(hybrid_temps) == len(days):
            ax.plot(days, hybrid_temps, '^-', label='Hybrid Forecast', color='green', linewidth=2)
    ...
    def _update_confidence_chart(self):
        ...
        ml_predictions = self.current_forecast.ml_predictions if self.current_forecast else []
    ...
    def _update_comparison_chart(self):
        ...
        if service_status.prediction_accuracy:
            ...
            ax.text(0.5, 0.5, 'No model performance data available', 
    ...
    def _update_details_display(self):
        ...
        details.append("=" * 50)
        details.append(f"Generated: {self.current_forecast.timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
        details.append(f"Overall Confidence: {self.current_forecast.confidence_score:.1%}")
        details.append("")
        ...
        if 'forecast_days' in self.current_forecast.api_forecast:
            for i, day_data in enumerate(self.current_forecast.api_forecast['forecast_days']):
        ...
        if self.current_forecast.ml_predictions:
            ...
            for i, pred in enumerate(self.current_forecast.ml_predictions):
                ...
                details.append(f"  Confidence Interval: {pred.confidence_interval[0]:.1f}°C - {pred.confidence_interval[1]:.1f}°C")
                details.append(f"  Weather Pattern: {pred.weather_pattern}")
                details.append(f"  Prediction Accuracy: {pred.prediction_accuracy:.1%}")
                details.append(f"  Features Used: {', '.join(pred.features_used[:5])}{'...' if len(pred.features_used) > 5 else ''}")
                details.append("")
        ...
        if service_status.last_training:
            details.append(f"Last Training: {service_status.last_training.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def _export_chart(self):
        ...
        try:
            ...
            if filename:
                # Get current tab
                current_tab = self.forecast_notebook.index(self.forecast_notebook.select())
        ...
        def run_async():
            try:
                ...
                explanation = loop.run_until_complete(
                    self.integration_service.get_prediction_explanation(self.current_city)
                )
                ...
                self.after(0, lambda: messagebox.showerror("Error", f"Failed to get model explanation: {str(e)}"))
    ...
    def _display_explanation(self, explanation):
        ...
        scrollbar = ttk.Scrollbar(text_frame, orient=tk.VERTICAL, command=explanation_text.yview)
        ...
        for model in explanation['models_used']:
            explanation_lines.append(f"  • {model['type']} (weight: {model['weight']:.3f})")
            explanation_lines.append(f"    Features: {', '.join(model['features_used'][:5])}")
        explanation_lines.append("")
        ...
        for model_type, metrics in explanation['accuracy_metrics'].items():
            ...
            explanation_lines.append(f"    Mean Absolute Error: {metrics['mean_absolute_error']}°C")
        explanation_lines.append("")
