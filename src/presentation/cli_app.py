...
from ..shared.exceptions import WeatherDashboardError, UIError
...
class WeatherDashboardCLIApp:
    ...
    def _inject_dependencies(self) -> None:
        ...
        try:
            ...
            self._comparison_service = self._container.get_service(ICityComparisonService)
            self._journal_service = self._container.get_service(IWeatherJournalService)
            self._activity_service = self._container.get_service(IActivitySuggestionService)
    ...
    def _create_argument_parser(self) -> argparse.ArgumentParser:
        ...
        parser = argparse.ArgumentParser(
            description="Weather Dashboard CLI",
        ...
        weather_parser.add_argument("--forecast", "-f", action="store_true", help="Include forecast")
        weather_parser.add_argument("--days", type=int, default=7, help="Forecast days (default: 7)")
        ...
        # Journal command
        journal_parser = subparsers.add_parser("journal", help="Weather journal operations")
        journal_subparsers = journal_parser.add_subparsers(dest="journal_action")
        
        # Journal create
        create_parser = journal_subparsers.add_parser("create", help="Create journal entry")
        ...
        list_parser.add_argument("--limit", type=int, default=10, help="Number of entries")
        
        # Activity command
        activity_parser = subparsers.add_parser("activities", help="Get activity suggestions")
        ...
        poetry_parser.add_argument("--style", default="haiku", help="Poetry style (default: haiku)")
    ...
    def _print_forecast_data(self, forecast_data: List[Dict[str, Any]]) -> None:
        ...
        for day in forecast_data:
            print(f"{day.get('date', 'N/A')}: {day.get('temperature', 'N/A')}°C, {day.get('condition', 'N/A')}")
    ...
    def _print_journal_entries(self, entries: List[Dict[str, Any]]) -> None:
        ...
        for entry in entries:
            print(f"[{entry.get('date', 'N/A')}] {entry.get('city', 'N/A')}: {entry.get('content', 'N/A')}")
    ...
    def _print_activity_suggestions(self, suggestions: List[Dict[str, Any]]) -> None:
        ...
        for i, suggestion in enumerate(suggestions, 1):
            print(f"{i}. {suggestion.get('name', 'N/A')} - {suggestion.get('description', 'N/A')}")
