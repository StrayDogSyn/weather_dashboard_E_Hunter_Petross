...
from ..models.capstone_models import (
    Activity, ActivitySuggestion, ActivityType, DEFAULT_ACTIVITIES
)
...
class ActivitySuggestionService:
    ...
    def __init__(self):
        ...
        self.logger.info(f"Activity service initialized with {len(self.activities)} default activities")
    ...
    def get_activity_suggestions(
        ...
        """
        self.logger.info(f"Getting activity suggestions for {weather.location.display_name}")
        ...
        suggestion = ActivitySuggestion(
            weather=weather,
            suggested_activities=top_suggestions
        )
    ...
    def get_indoor_activities(self, weather: CurrentWeather) -> List[Tuple[Activity, float]]:
        """
    ...
    def get_outdoor_activities(self, weather: CurrentWeather) -> List[Tuple[Activity, float]]:
        """
    ...
    def get_weather_suitability_explanation(
        ...
        if activity.indoor:
            ...
            # Temperature checks
            if activity.min_temperature is not None and temp_c < activity.min_temperature:
                explanations.append(f"Too cold (needs ≥{activity.min_temperature}°C, current: {temp_c:.1f}°C)")
            elif activity.max_temperature is not None and temp_c > activity.max_temperature:
                explanations.append(f"Too hot (needs ≤{activity.max_temperature}°C, current: {temp_c:.1f}°C)")
            else:
                explanations.append(f"Temperature is suitable ({temp_c:.1f}°C)")
            
            # Wind checks
            if activity.max_wind_speed is not None and weather.wind.speed > activity.max_wind_speed:
                explanations.append(f"Too windy (needs ≤{activity.max_wind_speed}km/h, current: {weather.wind.speed}km/h)")
            else:
                explanations.append(f"Wind conditions are acceptable ({weather.wind.speed}km/h)")
            
            # Weather condition checks
            if weather.condition in activity.ideal_conditions:
                explanations.append(f"Perfect weather conditions ({weather.condition.value})")
            elif activity.requires_clear_weather and weather.condition != WeatherCondition.CLEAR:
                explanations.append(f"Requires clear weather (current: {weather.condition.value})")
            else:
                explanations.append(f"Weather conditions are acceptable ({weather.condition.value})")
        
        suitability = "Highly suitable" if score >= 8 else "Suitable" if score >= 5 else "Moderately suitable"
    ...
    def get_activity_statistics(self, weather_history: List[CurrentWeather]) -> Dict[str, Any]:
        """
        ...
        most_common_condition = max(condition_counts.items(), key=lambda x: x[1]) if condition_counts else None
        
        return {
            'total_weather_records': len(weather_history),
            'activity_suggestions': activity_counts,
            'weather_conditions': condition_counts,
            'most_suggested_activity': most_common_activity[0] if most_common_activity else None,
            'most_common_condition': most_common_condition[0] if most_common_condition else None,
            'total_unique_activities': len(activity_counts),
            'average_suggestions_per_weather': sum(activity_counts.values()) / len(weather_history) if weather_history else 0
        }
    ...
    def create_activity_report(self, weather: CurrentWeather) -> str:
        ...
        if not suggestions.suggested_activities:
            lines.append("😕 No suitable activities found for current weather conditions.")
            return "\n".join(lines)
