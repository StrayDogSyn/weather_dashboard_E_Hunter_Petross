...
class ComparisonStrategy(ABC):
    ...
    @abstractmethod
    def calculate_score(
        self, weather: CurrentWeather, preferences: Optional[Dict[str, Any]] = None
    ) -> float:
...
class DefaultComparisonStrategy(ComparisonStrategy):
    ...
    def calculate_score(
        ...
        # Condition score
        condition_preferences = prefs.get(
            "preferred_conditions", [WeatherCondition.CLEAR]
        )
...
class WeatherComparison(AIEnhancedModel):
    ...
    def _calculate_all_metrics(self) -> None:
        ...
        self.custom_metrics.update(
            {
                "city1_score": self.strategy.calculate_score(
                    self.city1_weather, self.user_preferences
                ),
                "city2_score": self.strategy.calculate_score(
                    self.city2_weather, self.user_preferences
                ),
...
class WeatherMoodCorrelation:
    ...
    def matches_weather(self, weather: CurrentWeather) -> bool:
        ...
        return (
            ...
            and self.temperature_range[0] <= temp_c <= self.temperature_range[1]
        )
...
class JournalEntry(AIEnhancedModel):
    ...
    def add_activity(
        self, activity: str, category: Optional[ActivityType] = None
    ) -> None:
    ...
    def _get_activity_suggestions(self) -> List[str]:
        ...
        if self.energy_level >= 7:
            suggestions.extend(
                ["Try a new outdoor adventure", "Engage in physical exercise"]
            )
        elif self.energy_level <= 3:
            suggestions.extend(
                ["Practice relaxation techniques", "Enjoy quiet indoor activities"]
            )
        ...
        if self.mood in [MoodType.SAD, MoodType.ANXIOUS]:
            ...
            suggestions.extend(
                ["Channel energy into creative projects", "Try new experiences"]
            )
...
class ActivityRequirements:
    ...
    preferred_time_of_day: List[str] = field(
    ...
    def check_weather_compatibility(
        ...
        if self.min_temperature is not None and temp_c < self.min_temperature:
            issues.append(
                f"Temperature too low ({temp_c:.1f}°C < {self.min_temperature}°C)"
            )
        if self.max_temperature is not None and temp_c > self.max_temperature:
            issues.append(
                f"Temperature too high ({temp_c:.1f}°C > {self.max_temperature}°C)"
            )

        if self.max_wind_speed is not None and weather.wind.speed > self.max_wind_speed:
            issues.append(
                f"Wind too strong ({weather.wind.speed} km/h > {self.max_wind_speed} km/h)"
            )

        if self.max_humidity is not None and weather.humidity > self.max_humidity:
            issues.append(
                f"Humidity too high ({weather.humidity}% > {self.max_humidity}%)"
            )
...
class Activity(AIEnhancedModel):
    ...
    def is_suitable_for_weather(
        self, weather: CurrentWeather, user_preferences: Optional[Dict[str, Any]] = None
        ...
        if self.indoor:
            ...
            if weather.condition in self.ideal_conditions:
                ...
                score += 3.0
                notes.append(
                    f"Weather condition {weather.condition.value} is manageable"
                )
            ...
            if temp_pref_min <= temp_c <= temp_pref_max:
                ...
                self.requirements.min_temperature and self.requirements.max_temperature
            ):
                optimal_temp = (
                    self.requirements.min_temperature
                    + self.requirements.max_temperature
                ) / 2
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Activity":
        ...
        return cls(
            ...
            ideal_conditions=[
                WeatherCondition(c) for c in data.get("ideal_conditions", [])
            ],
...
class ActivitySuggestion(AIEnhancedModel):
    ...
    id: UUID = field(default_factory=uuid4)
    suggested_activities: List[Tuple[Activity, float, List[str]]] = field(
    ...
    def _generate_basic_suggestions(self) -> None:
        ...
        for activity in default_activities:
            is_suitable, score, notes = activity.is_suitable_for_weather(
                self.weather, self.user_preferences
            )
    ...
    def filter_by_difficulty(
        self, difficulty: ActivityDifficulty
    ) -> List[Tuple[Activity, float, List[str]]]:
    ...
    def _parse_ai_suggestions(self, ai_text: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if line.startswith(("1.", "2.", "3.", "4.", "5.")):
                ...
                current_suggestion
                and line
                and not line.startswith(("Tips:", "Unique:"))
            ):
...
class WeatherPoem(AIEnhancedModel):
    ...
    def _analyze_text_complexity(self) -> None:
        ...
        avg_word_length = sum(len(word.strip(".,!?;:")) for word in words) / max(
            len(words), 1
        )
        ...
        if sentence_count > 0:
            complexity += min(
                1.0, len(words) / sentence_count / 15
    ...
    def display_title(self) -> str:
        ...
        if self.location_context:
            ...
            return (
                f"{self.weather_condition.value.title()} {self.poem_type.value.title()}"
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mood_inspiration": (
                self.mood_inspiration.value if self.mood_inspiration else None
            ),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "WeatherPoem":
        ...
        return cls(
            ...
            mood_inspiration=(
                MoodType(data["mood_inspiration"])
                if data.get("mood_inspiration")
                else None
            ),
...
class ActivityFactory:
    ...
    def create_activity(
        self, name: str, description: str, activity_type: ActivityType, **kwargs
    ) -> Activity:
        ...
        activity = Activity(
            name=name, description=description, activity_type=activity_type, **kwargs
        )
    ...
    def create_from_ai_suggestion(
        self, suggestion_text: str, weather: CurrentWeather
    ) -> Optional[Activity]:
    ...
    def _parse_ai_activity_response(
...
class WeatherComparisonBuilder:
    ...
    def with_cities(
        self, city1_weather: CurrentWeather, city2_weather: CurrentWeather
...
def _determine_weather_impact(weather: CurrentWeather, mood: MoodType) -> WeatherImpact:
    ...
    if mood in positive_moods and base_impact in [
        WeatherImpact.POSITIVE,
        WeatherImpact.VERY_POSITIVE,
    ]:
        return WeatherImpact.VERY_POSITIVE
