...
class AIEnhancedWeatherModel(ABC):
    ...
    @abstractmethod
    def generate_ai_insights(self, gemini_api_key: Optional[str] = None) -> Dict[str, Any]:
...
class CurrentWeather(AIEnhancedWeatherModel):
    ...
    def activity_suitability(self) -> Dict[str, str]:
        ...
        return {
            ...
            "water_activities": "good" if 20 <= temp_c <= 30 and not self.is_severe_weather else "poor",
            "winter_sports": "excellent" if self.condition == WeatherCondition.SNOW else "poor"
        }
...
Format as JSON with keys: activities, health_safety, short_term_outlook, recommendations."""
        ...
        if ai_response:
            try:
                ...
                return {"status": "success", "raw_insights": ai_response, "generated_at": datetime.now(timezone.utc).isoformat()}
...
class WeatherFactory:
    ...
    def create_location(self, name: str, country: str, latitude: float, longitude: float, **kwargs) -> Location:
        ...
        if self.gemini_api_key:
            try:
                ...
                self.logger.info(f"Generated AI insights for location: {location.display_name}")
            except Exception as e:
    ...
    def create_current_weather(
        ...
        **kwargs
        ...
        temperature = Temperature(temperature_value, temperature_unit, kwargs.get('feels_like'))
        pressure = AtmosphericPressure(
            pressure_value,
        ...
        wind = Wind(
            wind_speed,
            kwargs.get('wind_direction'),
            kwargs.get('wind_gust')
        )
        ...
        if self.gemini_api_key:
            try:
                ...
                self.logger.info(f"Generated AI insights for weather: {location.display_name}")
            except Exception as e:
...
class CurrentWeatherBuilder:
    ...
    def with_temperature(self, value: float, unit: TemperatureUnit = TemperatureUnit.CELSIUS, feels_like: Optional[float] = None) -> 'CurrentWeatherBuilder':
    ...
    def with_condition(self, condition: WeatherCondition, description: str = "") -> 'CurrentWeatherBuilder':
        """Set weather condition."""
        self._condition = condition
        self._description = description or condition.value
        return self
    
    def with_atmosphere(self, humidity: int, pressure: float) -> 'CurrentWeatherBuilder':
        """Set atmospheric conditions."""
        self._humidity = humidity
        self._pressure_value = pressure
        return self
    
    def with_wind(self, speed: float, direction: Optional[int] = None) -> 'CurrentWeatherBuilder':
        """Set wind conditions."""
...
def create_location_from_dict(data: Dict[str, Any], gemini_api_key: Optional[str] = None) -> Location:
    ...
    return factory.create_location(
        ...
        **{k: v for k, v in data.items() if k not in ['name', 'country', 'latitude', 'longitude']}
    )


def create_weather_from_api_response(response_data: Dict[str, Any], gemini_api_key: Optional[str] = None) -> CurrentWeather:
    ...
    return factory.create_current_weather(
        ...
        visibility=response_data.get('visibility', 0) / 1000 if response_data.get('visibility') else None,  # Convert m to km
