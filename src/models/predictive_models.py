...
class WeatherPredictor:
    ...
    def prepare_features(self, weather_data: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        for window in [3, 6, 12, 24]:  # hours
            if len(df) >= window:
                df[f'temp_mean_{window}h'] = df['temp_celsius'].rolling(window=window).mean()
                df[f'temp_std_{window}h'] = df['temp_celsius'].rolling(window=window).std()
                if 'humidity' in df.columns:
                    df[f'humidity_mean_{window}h'] = df['humidity'].rolling(window=window).mean()
                if 'pressure' in df.columns:
                    df[f'pressure_mean_{window}h'] = df['pressure'].rolling(window=window).mean()
    ...
    def train_models(self, training_data: pd.DataFrame, target_column: str = 'temp_celsius') -> Dict[str, ModelMetrics]:
        ...
        self.feature_columns = [col for col in training_data.columns if col not in exclude_cols]
    ...
    def predict_temperature(self, 
        ...
        for hour in range(1, hours_ahead + 1):
            ...
            features = self._create_prediction_features(current_conditions, prediction_time)
    ...
    def _create_prediction_features(self, conditions: Dict[str, Any], prediction_time: datetime) -> List[float]:
        ...
        if 'weather_pattern' in self.encoders:
            ...
            try:
                encoded_pattern = self.encoders['weather_pattern'].transform([pattern])[0]
            except ValueError:
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        for name, model in self.models.items():
            if hasattr(model, 'feature_importances_'):
                model_importance = dict(zip(self.feature_columns, model.feature_importances_))
    ...
    def generate_model_report(self) -> str:
        ...
        if not self.training_data.empty:
            ...
            report.append(f"  • Date range: {self.training_data['timestamp'].min()} to {self.training_data['timestamp'].max()}")
