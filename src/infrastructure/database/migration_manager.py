...
class MigrationManager:
    ...
    def __init__(self, db_manager: DatabaseConnectionManager, migrations_dir: Optional[Path] = None):
    ...
    async def load_file_migrations(self) -> None:
        ...
        for file_path in migration_files:
            try:
                ...
                if len(parts) != 2:
                    logger.warning(f"Skipping migration file with invalid name: {filename}")
                    continue
                ...
                existing = next((m for m in self.migrations if m.version == version), None)
                if existing:
                    logger.warning(f"Migration {version} already exists, skipping file: {filename}")
                    continue
            ...
            async with self.db_manager.get_transaction() as conn:
                ...
                await conn.execute(
                    ...
                    (migration.version, migration.name, migration.checksum, 
                     datetime.now().isoformat(), execution_time)
                )
    ...
    async def rollback_migration(self, migration: Migration) -> None:
        ...
        try:
            ...
            raise DatabaseError(f"Rollback of migration {migration.version} failed: {e}")
    ...
    async def migrate_down(self, target_version: str) -> int:
        ...
        for migration_info in to_rollback:
            ...
            if not migration:
                logger.error(f"Migration {migration_info['version']} not found for rollback")
                continue
    ...
    async def validate_migrations(self) -> List[str]:
        ...
        for applied_migration in applied:
            ...
            if not migration:
                issues.append(f"Applied migration {version} not found in available migrations")
                continue
            
            if migration.checksum != stored_checksum:
                issues.append(f"Migration {version} checksum mismatch - migration may have been modified")
