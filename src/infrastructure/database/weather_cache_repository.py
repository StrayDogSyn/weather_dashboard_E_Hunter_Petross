...
class WeatherCacheRepository(BaseRepository[WeatherCacheModel]):
    ...
    async def get_by_location(
        self, location_name: str, data_type: str, max_age_hours: Optional[int] = None
    ) -> List[WeatherCacheModel]:
        """Get weather data for a location and data type."""
        cache_key = (
            f"{self.cache_prefix}:location:{location_name}:{data_type}:{max_age_hours}"
        )
        ...
        try:
            ...
            query = (
                f"{self._get_select_query()} WHERE location_name = ? AND data_type = ?"
            )
            ...
            logger.error(
                f"Failed to get weather data by location '{location_name}': {e}"
            )
            raise DatabaseError(f"Failed to get weather data by location: {e}")

    async def get_by_coordinates(
        self, latitude: float, longitude: float, data_type: str, radius_km: float = 1.0
    ) -> List[WeatherCacheModel]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get weather data by coordinates ({latitude}, {longitude}): {e}"
            )
    ...
    async def cache_weather_data(
        ...
        try:
            cache_key = self._generate_cache_key(
                location_name, latitude, longitude, data_type, params
            )
            ...
            async with self.db_manager.get_connection() as conn:
                ...
                for entry in expired_entries:
                    await self._clear_cache_pattern(
                        f"{self.cache_prefix}:key:{entry[1]}*"
                    )
        ...
        try:
            ...
            async with self.db_manager.get_connection() as conn:
                ...
                for row in candidates:
                    ...
                    cursor = await conn.execute(
                        "DELETE FROM weather_cache WHERE id = ?", (row[0],)
                    )
        ...
        try:
            async with self.db_manager.get_connection() as conn:
                ...
                return {
                    ...
                    "redis_hit_rate": redis_stats.get("keyspace_hits", 0)
                    / max(
    ...
    async def _update_access_stats(self, cache_id: int) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Failed to update access stats for cache entry {cache_id}: {e}"
            )
    ...
    async def get_archival_candidates(
        self, days_old: int = 30, min_access_count: int = 0
    ) -> List[WeatherCacheModel]:
