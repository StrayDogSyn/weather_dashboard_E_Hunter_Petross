...
class FavoritesRepository(BaseRepository[FavoriteLocationModel]):
    ...
    async def get_by_coordinates(self, latitude: float, longitude: float, tolerance: float = 0.001) -> Optional[FavoriteLocationModel]:
        ...
        try:
            ...
            logger.error(f"Failed to get favorite by coordinates ({latitude}, {longitude}): {e}")
    ...
    async def update_display_order(self, location_orders: List[tuple[int, int]]) -> bool:
        ...
        try:
            async with self.db_manager.get_transaction() as conn:
                ...
                logger.info(f"Updated display order for {len(location_orders)} locations")
                return True
    ...
    async def search_by_name(self, query: str, limit: int = 10) -> List[FavoriteLocationModel]:
        ...
        try:
            ...
            async with self.db_manager.get_connection() as conn:
                ...
                models = [self._extract_model_from_row(row) for row in rows]
                await self._set_list_cache(cache_key, models, ttl=300)  # Shorter TTL for search results
                return models
    ...
    async def get_by_tags(self, tags: List[str], match_all: bool = False) -> List[FavoriteLocationModel]:
        ...
        try:
            ...
            if match_all:
                ...
                filtered = [
                    ...
                    if all(tag in fav.tags for tag in tags)
                ]
            else:
                # Location must have ANY of the specified tags
                filtered = [
                    fav for fav in all_favorites
                    if any(tag in fav.tags for tag in tags)
                ]
        ...
        try:
            async with self.db_manager.get_connection() as conn:
                ...
                if cursor.rowcount > 0:
                    ...
                    await self._clear_cache_pattern(f"{self.cache_prefix}:{location_id}*")
                    await self._clear_cache_pattern(f"{self.cache_prefix}:all*")
            ...
            logger.error(f"Failed to update access info for location {location_id}: {e}")
    ...
    async def get_recently_accessed(self, limit: int = 5) -> List[FavoriteLocationModel]:
    ...
    async def validate_location(self, model: FavoriteLocationModel) -> List[str]:
        ...
        if existing and existing.id != model.id:
            errors.append(f"Location with similar coordinates already exists: {existing.name}")
    ...
    async def create_with_validation(self, model: FavoriteLocationModel) -> FavoriteLocationModel:
    ...
    async def update_with_validation(self, model: FavoriteLocationModel) -> FavoriteLocationModel:
