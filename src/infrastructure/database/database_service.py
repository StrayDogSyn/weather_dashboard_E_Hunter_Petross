...
from .connection_manager import ConnectionManagerFactory, DatabaseConnectionManager, RedisConnectionManager
...
from .weather_cache_repository import WeatherCacheRepository, create_weather_cache_repository
...
class DatabaseService:
    ...
    async def initialize(self, run_migrations: bool = True) -> None:
        ...
        try:
            ...
            # Initialize repositories
            self._favorites_repo = await create_favorites_repository(self.db_manager, self.redis_manager)
            self._journal_repo = await create_journal_repository(self.db_manager, self.redis_manager)
            self._settings_repo = await create_settings_repository(self.db_manager, self.redis_manager)
            self._weather_cache_repo = await create_weather_cache_repository(self.db_manager, self.redis_manager)
    ...
    def _ensure_initialized(self) -> None:
        ...
        if not self._initialized:
            raise DatabaseError("Database service not initialized. Call initialize() first.")
    ...
    async def cleanup_expired_data(self) -> Dict[str, int]:
        ...
        try:
            ...
            try:
                redis_info = await self.redis_manager.get_info()
                cleanup_results['redis_keys_cleaned'] = redis_info.get('expired_keys', 0)
            except Exception as e:
    ...
    async def backup_data(self, backup_path: str, include_cache: bool = False) -> Dict[str, Any]:
    ...
    async def optimize_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_info['size_reduction'] = optimization_info['before_size'] - optimization_info['after_size']
...
async def get_database_service() -> DatabaseService:
    ...
    if _database_service is None:
        raise DatabaseError("Database service not initialized. Call initialize_database_service() first.")
