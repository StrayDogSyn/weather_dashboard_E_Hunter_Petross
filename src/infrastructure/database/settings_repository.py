...
class SettingsRepository(BaseRepository[SettingsModel]):
        ...
        try:
            ...
            async with self.db_manager.get_connection() as conn:
                cursor = await conn.execute(
                    f"{self._get_select_query()} WHERE setting_key = ?", (setting_key,)
                )
    ...
    async def get_value(self, setting_key: str, default: Any = None) -> Any:
        ...
        if setting:
            return self._convert_setting_value(
                setting.setting_value, setting.setting_type
            )
        return default

    async def set_value(
        self, setting_key: str, value: Any, setting_type: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            if existing:
                ...
                await self._clear_cache_pattern(
                    f"{self.cache_prefix}:key:{setting_key}*"
                )
                await self._clear_cache_pattern(
                    f"{self.cache_prefix}:category:{existing.category}*"
                )
    ...
    async def reset_to_default(self, setting_key: str) -> bool:
        ...
        try:
            ...
            if setting.default_value is not None:
                ...
                await self._clear_cache_pattern(
                    f"{self.cache_prefix}:category:{setting.category}*"
                )
    ...
    async def export_settings(
        self, category: Optional[str] = None, user_configurable_only: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for setting in settings:
                export_data["settings"][setting.setting_key] = {
                    "value": self._convert_setting_value(
                        setting.setting_value, setting.setting_type
                    ),
    ...
    async def import_settings(
        self, settings_data: Dict[str, Any], overwrite_existing: bool = False
    ) -> int:
            ...
            for setting_key, setting_info in settings_data["settings"].items():
                ...
                if existing:
                    ...
                    new_setting = SettingsModel(
                        ...
                        description=setting_info.get(
                            "description", f"Imported setting: {setting_key}"
                        ),
                        is_user_configurable=setting_info.get(
                            "is_user_configurable", True
                        ),
    ...
    def _convert_setting_value(self, value: str, setting_type: str) -> Any:
        ...
        try:
            ...
            logger.warning(
                f"Failed to convert setting value '{value}' to type '{setting_type}': {e}"
            )
            return value
    ...
    async def validate_setting(self, model: SettingsModel) -> List[str]:
        ...
        if (
            model.setting_key
            and not model.setting_key.replace("_", "").replace(".", "").isalnum()
        ):
            errors.append(
                "Setting key must contain only alphanumeric characters, underscores, and dots"
            )
        ...
        if model.setting_type:
            try:
                self._convert_setting_value(
                    str(model.setting_value), model.setting_type
                )
            except Exception:
                errors.append(
                    f"Setting value is not compatible with type '{model.setting_type}'"
                )
    ...
    async def create_with_validation(self, model: SettingsModel) -> SettingsModel:
        ...
        if existing:
            raise ValidationError(
                f"Setting with key '{model.setting_key}' already exists"
            )
