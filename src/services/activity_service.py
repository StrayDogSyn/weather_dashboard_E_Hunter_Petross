...
from ..interfaces.activity_interfaces import (
    IActivityService, IActivityDatabase, IWeatherMatcher, IPreferenceMatcher,
    IRecommendationEngine, Activity, ActivityRequest, ActivityResponse,
    ActivityRecommendation, WeatherConditions, ActivityPreferences,
    ActivityFilter, WeatherRequirement, ActivityCategory, ActivityDifficulty,
    ActivityDuration, WeatherSuitability
)
...
class InMemoryActivityDatabase(IActivityDatabase):
    ...
    async def search_activities(self, filter_criteria: ActivityFilter) -> List[Activity]:
        ...
        if filter_criteria.categories:
            activities = [a for a in activities if a.category in filter_criteria.categories]
        
        if filter_criteria.difficulty:
            activities = [a for a in activities if a.difficulty == filter_criteria.difficulty]
        
        if filter_criteria.duration:
            activities = [a for a in activities if a.duration == filter_criteria.duration]
        ...
        if filter_criteria.equipment_available:
            ...
            activities = [
                ...
                if not a.equipment_needed or set(a.equipment_needed).issubset(available_equipment)
            ]
        
        if filter_criteria.group_size:
            activities = [
                a for a in activities 
                if a.min_participants <= filter_criteria.group_size and 
                (a.max_participants is None or filter_criteria.group_size <= a.max_participants)
            ]
    ...
    def _initialize_default_activities(self):
        ...
        default_activities = [
            ...
            Activity(
                ...
                cost_estimate="low",
                location_type="park",
                safety_considerations=["check weather conditions", "inform someone of your route"]
            ),
            Activity(
                id="cycling",
                name="Cycling",
            ...
            Activity(
                ...
                cost_estimate="high",
                location_type="venue",
                safety_considerations=["proper training required", "use safety equipment"]
            ),
...
class WeatherMatcher(IWeatherMatcher):
    ...
    async def generate_weather_warnings(
        ...
        warnings = []
        requirements = await self.get_weather_requirements(activity)
...
class PreferenceMatcher(IPreferenceMatcher):
    ...
    async def calculate_preference_score(
        ...
        if preferences.group_size:
            ...
            if (activity.min_participants <= preferences.group_size and 
                (activity.max_participants is None or preferences.group_size <= activity.max_participants)):
                score += weight
    ...
    async def filter_by_preferences(
        ...
        if preferences.preferred_categories:
            filtered = [a for a in filtered if a.category in preferences.preferred_categories]
        
        # Filter by difficulty
        if preferences.difficulty_level:
            filtered = [a for a in filtered if a.difficulty == preferences.difficulty_level]
        ...
        if preferences.indoor_preference is not None:
            filtered = [a for a in filtered if a.indoor == preferences.indoor_preference]
        
        # Filter by group size
        if preferences.group_size:
            filtered = [
                a for a in filtered 
                if a.min_participants <= preferences.group_size and 
                (a.max_participants is None or preferences.group_size <= a.max_participants)
            ]
    ...
    async def get_preference_reasons(
        ...
        if preferences.preferred_categories and activity.category in preferences.preferred_categories:
            reasons.append(f"Matches your interest in {activity.category.value.replace('_', ' ')}")
        
        if preferences.difficulty_level and activity.difficulty == preferences.difficulty_level:
        ...
        if preferences.group_size:
            if activity.min_participants <= preferences.group_size:
                if activity.max_participants is None:
                    reasons.append(f"Suitable for groups of {preferences.group_size} or more")
                elif preferences.group_size <= activity.max_participants:
                    reasons.append(f"Perfect for your group size of {preferences.group_size}")
...
class RecommendationEngine(IRecommendationEngine):
    ...
    def __init__(
        self,
        weather_matcher: IWeatherMatcher,
        preference_matcher: IPreferenceMatcher
    ):
    ...
    async def rank_activities(
        ...
        for activity in activities:
            ...
            weather_score = await self.weather_matcher.get_weather_score(activity, weather, weather_req)
            weather_suitability = await self.weather_matcher.calculate_weather_suitability(
...
class ActivityService(IActivityService):
    ...
    async def get_recommendations(self, request: ActivityRequest) -> ActivityResponse:
        ...
        try:
            ...
            if request.include_indoor_backup:
                outdoor_activities = [r.activity for r in recommendations if not r.activity.indoor]
                if outdoor_activities:
                    indoor_alternatives = await self.recommendation_engine.get_indoor_alternatives(
            ...
            general_advice = self._generate_general_advice(request.weather_conditions, recommendations)
    ...
    async def search_activities(
        ...
        for activity in all_activities:
            ...
            if (query_lower in activity.name.lower() or 
                query_lower in activity.description.lower()):
    ...
    async def get_weather_advice(
        ...
        # Generate preparation tips
        prep_tips = await self.recommendation_engine.generate_preparation_tips(activity, weather)
    ...
    async def get_seasonal_activities(
        ...
        seasonal_activities = [
            ...
            if not activity.seasonal_availability or season.lower() in activity.seasonal_availability
        ]
    ...
    async def get_trending_activities(
        ...
        for activity in all_activities:
            weather_req = await weather_matcher.get_weather_requirements(activity)
            score = await weather_matcher.get_weather_score(activity, weather, weather_req)
