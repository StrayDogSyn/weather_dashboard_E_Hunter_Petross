...
from ..models.predictive_models import WeatherPredictor, ModelType, PredictionResult, ModelMetrics
...
class ModelTrainingService:
    ...
    def __init__(self, config_mgr, data_storage: IDataStorage, weather_api: IWeatherAPI):
    ...
    async def collect_training_data(self, days: int = 30) -> pd.DataFrame:
        ...
        required_columns = ['timestamp', 'temperature', 'humidity', 'pressure', 'description']
    ...
    async def train_all_models(self, force_retrain: bool = False) -> Dict[str, TrainingResult]:
        ...
        training_data = await self.collect_training_data(self.training_config.training_days)
        
        if len(training_data) < self.training_config.min_data_points:
            raise ValueError(f"Insufficient training data: {len(training_data)} < {self.training_config.min_data_points}")
    ...
    async def _train_single_model(self, model_type: ModelType, training_data: pd.DataFrame) -> TrainingResult:
        ...
        try:
            ...
            self.logger.info(f"Successfully trained {model_type.value} model (R²: {validation_score:.3f})")
            return result
    ...
    def _should_retrain(self) -> bool:
        ...
        if days_since_training >= self.training_config.auto_retrain_days:
            self.logger.info(f"Models are {days_since_training} days old, retraining needed")
            return True
        
        # Check performance
        if latest_training.validation_score < self.training_config.retrain_threshold:
            self.logger.info(f"Model performance below threshold ({latest_training.validation_score:.3f} < {self.training_config.retrain_threshold})")
            return True
        
        return False
    
    async def validate_models(self, test_data: Optional[pd.DataFrame] = None) -> Dict[str, float]:
            ...
            try:
                ...
                for _, model in predictor.models.items():
                    if hasattr(model, 'predict'):
                        try:
                            if 'linear_regression' in predictor.models and model == predictor.models['linear_regression']:
                                X_scaled = predictor.scalers['features'].transform(X_test)
    ...
    async def _generate_training_report(self, results: Dict[str, TrainingResult], training_data: pd.DataFrame):
        ...
        report_lines.append(f"- Training period: {self.training_config.training_days} days")
        ...
        for model_type, result in results.items():
            ...
            report_lines.append(f"- Training duration: {result.training_duration:.2f} seconds")
            report_lines.append(f"- Validation score (R²): {result.validation_score:.3f}")
            ...
            if result.model_metrics:
                ...
                for sub_model, metrics in result.model_metrics.items():
                    report_lines.append(f"  - {sub_model}: MAE={metrics.mae:.2f}, RMSE={metrics.rmse:.2f}, R²={metrics.r2:.3f}")
        ...
        if not training_data.empty:
            report_lines.append(f"- Temperature range: {training_data['temperature'].min():.1f}°C to {training_data['temperature'].max():.1f}°C")
            report_lines.append(f"- Average temperature: {training_data['temperature'].mean():.1f}°C")
            report_lines.append(f"- Humidity range: {training_data['humidity'].min():.1f}% to {training_data['humidity'].max():.1f}%")
            report_lines.append(f"- Pressure range: {training_data['pressure'].min():.1f} hPa to {training_data['pressure'].max():.1f} hPa")
        ...
        best_model = max(results.items(), key=lambda x: x[1].validation_score) if results else None
        if best_model:
            report_lines.append(f"- Best performing model: {best_model[0]} (R²: {best_model[1].validation_score:.3f})")
        
        failed_models = [name for name, result in results.items() if result.status == "failed"]
        if failed_models:
            report_lines.append(f"- Failed models need attention: {', '.join(failed_models)}")
        
        low_performance = [name for name, result in results.items() if result.validation_score < 0.6]
        if low_performance:
            report_lines.append(f"- Models with low performance: {', '.join(low_performance)}")
        
        if len(training_data) < 500:
            report_lines.append("- Consider collecting more training data for better model performance")
        
        # Save report
        report_content = "\n".join(report_lines)
        report_file = self.model_reports_path / f"training_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    ...
    def load_training_history(self):
        ...
        try:
            ...
            self.logger.debug(f"Loaded {len(self.training_history)} training history entries")
    ...
    def get_model_status(self) -> Dict[str, Any]:
        ...
        if self.training_history:
            ...
            next_retrain = latest_training.timestamp + timedelta(days=self.training_config.auto_retrain_days)
