...
class InMemoryJournalStorage(IJournalStorage):
    ...
    async def search_entries(
        ...
        try:
            ...
            if request.include_weather_stats and filtered_entries:
                ...
                weather_stats = await analyzer.calculate_weather_statistics(
                    filtered_entries
                )
    ...
    def _apply_filters(
        self, entries: List[JournalEntry], filters: JournalSearchFilter
    ) -> List[JournalEntry]:
        ...
        if filters.weather_conditions:
            filtered = [
                ...
                for e in filtered
                if any(
                    condition.lower() in e.weather_snapshot.condition.lower()
        ...
        if filters.min_temperature is not None:
            filtered = [
                e
                for e in filtered
                if e.weather_snapshot.temperature >= filters.min_temperature
            ]
        if filters.max_temperature is not None:
            filtered = [
                e
                for e in filtered
                if e.weather_snapshot.temperature <= filters.max_temperature
            ]
        ...
        if filters.locations:
            filtered = [
                ...
                if (
                    e.location
                    and any(
                        loc.lower() in e.location.lower() for loc in filters.locations
                    )
                )
                or (
                    e.weather_snapshot.location
                    and any(
                        loc.lower() in e.weather_snapshot.location.lower()
        ...
        if filters.min_rating is not None:
            filtered = [
                e for e in filtered if e.rating and e.rating >= filters.min_rating
            ]
        if filters.max_rating is not None:
            filtered = [
                e for e in filtered if e.rating and e.rating <= filters.max_rating
            ]
        ...
        if filters.text_query:
            ...
            filtered = [
                ...
                if (
                    ...
                    or (e.tags and any(query_lower in tag.lower() for tag in e.tags))
                )
            ]
    ...
    def _sort_entries(
        self, entries: List[JournalEntry], sort_by: SearchSortBy
    ) -> List[JournalEntry]:
        ...
        if sort_by == SearchSortBy.DATE_DESC:
            ...
            return sorted(
                entries, key=lambda e: e.weather_snapshot.temperature, reverse=True
            )
        else:
            return entries
...
class WeatherAnalyzer(IWeatherAnalyzer):
    ...
    async def calculate_weather_statistics(
        ...
        most_common_condition = (
            condition_counts.most_common(1)[0][0] if condition_counts else ""
        )
        ...
        sunny_days = sum(
            1
            for c in conditions
            if any(word in c for word in ["sunny", "clear", "bright"])
        )
        rainy_days = sum(
            1
            for c in conditions
            if any(word in c for word in ["rain", "drizzle", "shower"])
        )
    ...
    async def analyze_mood_patterns(self, entries: List[JournalEntry]) -> MoodAnalysis:
        ...
        challenging_weather_days = [
            e.date for e in rated_entries[-5:] if e.rating <= 2.0
        ]
    ...
    async def find_weather_correlations(
        ...
        for entry in entries:
            if entry.mood:
                temp_range = self._get_temperature_range(
                    entry.weather_snapshot.temperature
                )
        ...
        seasonal_data = defaultdict(
            lambda: {"moods": [], "activities": [], "ratings": []}
        )
        for entry in entries:
        ...
        for season, data in seasonal_data.items():
            correlations["seasonal_patterns"][season] = {
                ...
                "avg_rating": (
                    statistics.mean(data["ratings"]) if data["ratings"] else 0
                ),
            }
        ...
        if weather_stats.temperature_trend != "stable":
            insights.append(
                JournalInsight(
                    ...
                    confidence=0.8,
                    supporting_data={
                        "trend": weather_stats.temperature_trend,
        ...
        if mood_analysis.weather_mood_correlations:
            ...
            insights.append(
                JournalInsight(
                    ...
                    confidence=0.7,
                    supporting_data={
                        "weather": best_weather[0],
                        "mood": best_weather[1].value,
                    },
                    generated_at=datetime.now(),
                )
            )
    ...
    def _calculate_temperature_trend(self, entries: List[JournalEntry]) -> str:
        ...
        numerator = sum(
            (x - x_mean) * (y - y_mean) for x, y in zip(x_values, temperatures)
        )
    ...
    def _generate_seasonal_recommendation(
        ...
        seasonal_entries = [
            entry
            for entry in entries
            if self._get_season(entry.date) == current_season
            and entry.date.year < date.today().year
        ]
...
class JournalService(IJournalService):
    ...
    async def get_recent_entries(self, limit: int = 10) -> List[JournalEntry]:
        ...
        request = JournalSearchRequest(
            filters=JournalSearchFilter(), sort_by=SearchSortBy.DATE_DESC, limit=limit
        )
    ...
    async def get_entries_for_date(self, target_date: date) -> List[JournalEntry]:
        ...
        request = JournalSearchRequest(
            filters=JournalSearchFilter(start_date=target_date, end_date=target_date)
        )
        result = await self.search_entries(request)
        return result.entries
