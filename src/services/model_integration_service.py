...
from ..models.predictive_models import WeatherPredictor, ModelType, PredictionResult
from ..services.model_training_service import ModelTrainingService
from ..interfaces.weather_interfaces import IWeatherAPI, IDataStorage
...
class ModelIntegrationService:
    ...
    async def _initialize_models(self):
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(trained_models)} trained models: {trained_models}")
    ...
    async def _calculate_model_weights(self):
        ...
        latest_training = max(self.training_service.training_history, 
    ...
    async def get_enhanced_forecast(self, 
                                  city: str, 
                                  days: int = 5,
                                  include_ml_predictions: bool = True) -> IntegratedForecast:
        ...
        try:
            ...
            if include_ml_predictions and self.models_loaded:
                ...
                if ml_predictions:
                    ...
                    confidence_score = self._calculate_prediction_confidence(ml_predictions)
            
            # Create hybrid forecast
            hybrid_forecast = await self._create_hybrid_forecast(api_data, ml_predictions)
    ...
    async def _generate_ml_predictions(self, city: str, days: int) -> List[PredictionResult]:
    ...
    def _create_ensemble_prediction(self, 
                                  ...
                                  weights: List[float],
                                  prediction_date: datetime) -> PredictionResult:
        ...
        if total_weight == 0:
            ...
            normalized_weights = [w / total_weight for w in weights] if weights else [1.0 / len(predictions)] * len(predictions)
        
        # Weighted average of predictions
        weighted_temp = sum(p.predicted_temperature * w for p, w in zip(predictions, normalized_weights))
        
        # Average confidence (using prediction_accuracy as proxy)
        avg_confidence = sum(p.prediction_accuracy * w for p, w in zip(predictions, normalized_weights))
    ...
    async def _create_hybrid_forecast(self, 
                                    api_data: Dict[str, Any],
                                    ml_predictions: List[PredictionResult]) -> Dict[str, Any]:
            ...
            if 'forecast_days' in hybrid:
                for i, day_forecast in enumerate(hybrid['forecast_days']):
                    if i < len(ml_predictions):
                        ...
                        day_forecast['confidence_interval'] = ml_pred.confidence_interval
    ...
    def _calculate_prediction_confidence(self, predictions: List[PredictionResult]) -> float:
        ...
        variance_confidence = max(0.0, 1.0 - (temp_variance / 100.0))  # Normalize by expected temp range
        
        # Average individual confidences
        avg_confidence = sum(p.prediction_accuracy for p in predictions) / len(predictions)
        ...
        for model_type, predictor in self.predictors.items():
            if predictor.is_trained:
                model_info = {
                    'type': model_type,
                    'weight': self.model_weights.get(model_type, 0.0),
                    'features_used': list(predictor.feature_columns) if predictor.feature_columns else [],
                    'trained': True
                }
