...
class WeatherDataCollector(IWeatherDataCollector):
    ...
    async def collect_historical_data(
        ...
        logger.warning(
            "Historical data collection not fully implemented - using current data"
        )
    ...
    async def _fetch_current_weather(
        self, location: ComparisonLocation
    ...
    async def _fetch_forecast_weather(
        self, location: ComparisonLocation, days: int
    ) -> List[WeatherDataPoint]:
        """Fetch forecast weather for a single location."""
        try:
            forecast_data = await self.weather_api.get_forecast(
                lat=location.latitude, lon=location.longitude, days=days
            )
    ...
    async def generate_chart(self, config: ChartConfiguration) -> bytes:
        ...
        try:
            ...
            img_bytes = fig.to_image(
                format="png", width=config.width, height=config.height
            )
            return img_bytes
        except Exception as e:
...
class StatisticsCalculator(IStatisticsCalculator):
                    ...
                    for j, metric2 in enumerate(metrics):
                        if i <= j:  # Only calculate upper triangle
                            ...
                            if (
                                len(values1) > 1
                                and len(values2) > 1
                                and len(values1) == len(values2)
                            ):
                                corr = self._calculate_pearson_correlation(
                                    values1, values2
                                )
                                correlation_matrix[location][
                                    f"{metric1.value}_{metric2.value}"
                                ] = corr
    ...
    def _get_metric_value(
        self, point: WeatherDataPoint, metric: ComparisonMetric
    ) -> Optional[float]:
...
class InsightGenerator(IInsightGenerator):
    ...
    async def generate_insights(self, comparison_result: ComparisonResult) -> List[str]:
        ...
        try:
            ...
            temp_stats = next(
                (
                    s
                    for s in comparison_result.statistics
                    if s.metric == ComparisonMetric.TEMPERATURE
                ),
                None,
            )
            ...
            humidity_stats = next(
                (
                    s
                    for s in comparison_result.statistics
                    if s.metric == ComparisonMetric.HUMIDITY
                ),
                None,
            )
        ...
        try:
            ...
            if temp_stats:
                recommendations.extend(
                    self._generate_temperature_recommendations(temp_stats)
                )

            # Activity recommendations based on conditions
            recommendations.extend(
                self._generate_activity_recommendations(comparison_result)
            )
    ...
    def _generate_temperature_insights(self, stats: ComparisonStatistics) -> List[str]:
        ...
        if stats.avg_values:
            ...
            insights.append(
                f"{warmest} is the warmest location with an average of {stats.avg_values[warmest]:.1f}°C"
            )
            insights.append(
                f"{coolest} is the coolest location with an average of {stats.avg_values[coolest]:.1f}°C"
            )
            insights.append(
                f"Temperature difference between locations: {temp_diff:.1f}°C"
            )
    ...
    def _generate_humidity_insights(self, stats: ComparisonStatistics) -> List[str]:
        ...
        if stats.avg_values:
            ...
            insights.append(
                f"{most_humid} has the highest humidity at {stats.avg_values[most_humid]:.1f}%"
            )
            insights.append(
                f"{least_humid} has the lowest humidity at {stats.avg_values[least_humid]:.1f}%"
            )
    ...
    def _generate_general_insights(self, result: ComparisonResult) -> List[str]:
        ...
        insights.append(
            f"Comparison includes {len(result.request.locations)} locations"
        )
        insights.append(f"Data quality score: {result.data_quality_score:.1%}")

        if result.missing_data_points > 0:
            insights.append(
                f"Note: {result.missing_data_points} data points were missing"
            )
    ...
    def _generate_temperature_recommendations(
        ...
        if stats.avg_values:
            ...
            if stats.avg_values[warmest] > 25:
                recommendations.append(
                    f"Consider {warmest} for warm weather activities"
                )
    ...
    def _generate_activity_recommendations(self, result: ComparisonResult) -> List[str]:
        ...
        if temp_stats and temp_stats.avg_values:
            for location, temp in temp_stats.avg_values.items():
                if temp > 25:
                    recommendations.append(
                        f"{location}: Great for outdoor swimming and beach activities"
                    )
                elif temp > 15:
                    recommendations.append(
                        f"{location}: Perfect for hiking and outdoor sports"
                    )
                elif temp > 5:
                    recommendations.append(
                        f"{location}: Good for sightseeing and indoor activities"
                    )
                else:
                    recommendations.append(
                        f"{location}: Ideal for winter sports and cozy indoor activities"
                    )
...
class ComparisonService(IComparisonService):
    ...
    async def compare_weather(self, request: ComparisonRequest) -> ComparisonResult:
        ...
        try:
            ...
            if request.time_range == TimeRange.CURRENT:
                data_points = await self.data_collector.collect_current_data(
                    request.locations
                )
            elif request.time_range in [
                TimeRange.HOURLY_24H,
                TimeRange.DAILY_7D,
                TimeRange.DAILY_14D,
            ]:
                if request.include_forecast:
                    days = (
                        1
                        if request.time_range == TimeRange.HOURLY_24H
                        else (7 if request.time_range == TimeRange.DAILY_7D else 14)
                    )
                    data_points = await self.data_collector.collect_forecast_data(
                        request.locations, days
                    )
                else:
                    data_points = await self.data_collector.collect_current_data(
                        request.locations
            ...
            for metric in request.metrics:
                stats = await self.statistics_calculator.calculate_basic_stats(
                    data_points, metric
                )
            ...
            result = ComparisonResult(
                ...
                data_quality_score=self._calculate_data_quality_score(
                    data_points, request.locations
                ),
                missing_data_points=self._count_missing_data_points(
                    data_points, request.locations
                ),
            )

            insights = await self.insight_generator.generate_insights(result)
            recommendations = await self.insight_generator.generate_recommendations(
                result
            )
    ...
    async def get_location_suggestions(self, query: str) -> List[ComparisonLocation]:
        ...
        suggestions = [
            loc for loc in sample_locations if query_lower in loc.name.lower()
        ]
    ...
    async def _create_chart_config(
        self, request: ComparisonRequest, data_points: List[WeatherDataPoint]
    ) -> ChartConfiguration:
            ...
            for metric in request.metrics:
                ...
                if location_data:
                    ...
                    for point in location_data:
                        value = self.statistics_calculator._get_metric_value(
                            point, metric
                        )
                        if value is not None:
    ...
    def _calculate_data_quality_score(
        self, data_points: List[WeatherDataPoint], locations: List[ComparisonLocation]
    ) -> float:
    ...
    def _count_missing_data_points(
        self, data_points: List[WeatherDataPoint], locations: List[ComparisonLocation]
    ) -> int:
