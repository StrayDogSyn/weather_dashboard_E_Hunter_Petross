...
from ..models import (
    CurrentWeather, WeatherForecast, Location,
    Temperature, Wind, Precipitation, AtmosphericPressure,
    WeatherCondition, TemperatureUnit, WeatherForecastDay
)
...
class OpenWeatherMapAPI(IWeatherAPI):
    ...
    def _parse_weather_condition(self, weather_data: Dict[str, Any]) -> WeatherCondition:
    ...
    def get_forecast(self, city: str, days: int = 5, units: str = "metric") -> Optional[ForecastData]:
        """
    ...
    def _create_forecast_day(self, forecasts: List[Dict], units: str) -> Optional[WeatherForecastDay]:
        ...
        try:
            ...
            return WeatherForecastDay(
                ...
                condition=self._condition_map.get(most_common_condition, WeatherCondition.CLEAR),
                description=first_forecast['weather'][0]['description'],
                humidity=humidity,
                wind=Wind(speed=avg_wind_speed),
                precipitation_chance=int(forecasts[0].get('pop', 0) * 100),  # Probability of precipitation
