...
try:
    ...
    print("Matplotlib not available. Please install it with: pip install matplotlib numpy")
...
class WeatherVisualizationService:
    ...
    def create_weather_metrics_chart(self, parent: tk.Widget) -> tk.Widget:
        ...
        latest_pressure = (self.pressure_data[-1][1] / 10) if self.pressure_data else 0  # Scale pressure
    ...
    def create_forecast_comparison_chart(self, parent: tk.Widget, forecast: Optional[WeatherForecast] = None) -> tk.Widget:
        ...
        bars1 = ax.bar([xi - width/2 for xi in x], high_temps, width, label='High', color='#ff6b4a', alpha=0.8)
        bars2 = ax.bar([xi + width/2 for xi in x], low_temps, width, label='Low', color='#4a9eff', alpha=0.8)
    ...
    def create_humidity_pressure_chart(self, parent: tk.Widget) -> tk.Widget:
        ...
        if self.humidity_data:
            ...
            line1 = ax1.plot(times_hum, humidity_vals, color='#4a9eff', linewidth=2, 
        ...
        if self.pressure_data:
            ...
            line2 = ax2.plot(times_press, pressure_vals, color='#22c55e', linewidth=2, 
                           marker='s', markersize=3, label='Pressure (hPa)')
            ax2.set_ylabel('Pressure (hPa)', color='#22c55e')
            ax2.tick_params(axis='y', labelcolor='#22c55e')
        
        ax1.set_title('Humidity & Pressure Trends (24h)', color='white', fontsize=14, pad=20)
