...
class APIConfiguration:
    ...
    timeout: int = field(default_factory=lambda: int(os.getenv("WEATHER_API_TIMEOUT", "10")))
    max_retries: int = field(default_factory=lambda: int(os.getenv("WEATHER_MAX_RETRIES", "3")))
    rate_limit_per_minute: int = field(default_factory=lambda: int(os.getenv("WEATHER_RATE_LIMIT_PER_MINUTE", "60")))
    
    # Alternative APIs for future expansion
    weatherapi_key: str = field(default_factory=lambda: os.getenv("WEATHERAPI_KEY", ""))
    visualcrossing_key: str = field(default_factory=lambda: os.getenv("VISUALCROSSING_API_KEY", ""))
...
class UIConfiguration:
    ...
    window_title: str = field(default_factory=lambda: os.getenv("WEATHER_WINDOW_TITLE", "🌤️ Weather Dashboard"))
    min_size: tuple = field(default_factory=lambda: _parse_size(os.getenv("WEATHER_MIN_SIZE", "800,600")))
    default_size: tuple = field(default_factory=lambda: _parse_size(os.getenv("WEATHER_DEFAULT_SIZE", "1200,800")))
    update_interval: int = field(default_factory=lambda: int(os.getenv("WEATHER_UPDATE_INTERVAL", "30000")))
    enable_animations: bool = field(default_factory=lambda: os.getenv("WEATHER_ENABLE_ANIMATIONS", "true").lower() == "true")
...
class DataConfiguration:
    ...
    cache_duration: int = field(default_factory=lambda: int(os.getenv("WEATHER_CACHE_DURATION", "300")))
    auto_refresh: bool = field(default_factory=lambda: os.getenv("WEATHER_AUTO_REFRESH", "false").lower() == "true")
    auto_refresh_interval: int = field(default_factory=lambda: int(os.getenv("WEATHER_AUTO_REFRESH_INTERVAL", "300")))
    max_history_entries: int = field(default_factory=lambda: int(os.getenv("WEATHER_MAX_HISTORY_ENTRIES", "1000")))
    export_format: str = field(default_factory=lambda: os.getenv("WEATHER_EXPORT_FORMAT", "json"))
    backup_enabled: bool = field(default_factory=lambda: os.getenv("WEATHER_BACKUP_ENABLED", "true").lower() == "true")


@dataclass
class LoggingConfiguration:
    """Logging configuration settings."""
    log_level: str = field(default_factory=lambda: os.getenv("WEATHER_LOG_LEVEL", "INFO").upper())
    log_to_file: bool = True
    log_to_console: bool = True
    max_log_size: int = 10  # MB
    log_retention_days: int = 30
    performance_logging: bool = field(default_factory=lambda: os.getenv("WEATHER_PERFORMANCE_LOGGING", "true").lower() == "true")
    api_logging: bool = field(default_factory=lambda: os.getenv("WEATHER_API_LOGGING", "true").lower() == "true")
    ui_logging: bool = field(default_factory=lambda: os.getenv("WEATHER_UI_LOGGING", "false").lower() == "true")


@dataclass
class SecurityConfiguration:
    """Security configuration settings."""
    mask_api_keys: bool = field(default_factory=lambda: os.getenv("WEATHER_MASK_API_KEYS", "true").lower() == "true")
    log_api_responses: bool = field(default_factory=lambda: os.getenv("WEATHER_LOG_API_RESPONSES", "false").lower() == "true")
    privacy_mode: bool = field(default_factory=lambda: os.getenv("WEATHER_PRIVACY_MODE", "false").lower() == "true")


@dataclass
class FeatureConfiguration:
    """Feature flags configuration."""
    enable_ml_predictions: bool = field(default_factory=lambda: os.getenv("WEATHER_ENABLE_ML_PREDICTIONS", "false").lower() == "true")
    enable_weather_alerts: bool = field(default_factory=lambda: os.getenv("WEATHER_ENABLE_WEATHER_ALERTS", "true").lower() == "true")
    enable_location_services: bool = field(default_factory=lambda: os.getenv("WEATHER_ENABLE_LOCATION_SERVICES", "false").lower() == "true")
...
class ApplicationConfiguration:
    ...
    # User preferences
    default_city: str = field(default_factory=lambda: os.getenv("WEATHER_DEFAULT_CITY", "New York"))
    favorite_cities: list = field(default_factory=list)
    temperature_unit: str = "metric"  # metric, imperial, standard
    debug_mode: bool = field(default_factory=lambda: os.getenv("WEATHER_DEBUG", "false").lower() == "true")
...
class ConfigurationManager:
    ...
    def _validate_configuration(self) -> None:
        ...
        if self.config.temperature_unit not in ["metric", "imperial", "standard"]:
            errors.append("Temperature unit must be 'metric', 'imperial', or 'standard'")
    ...
    def get_masked_api_key(self) -> str:
        ...
        if self.config.security.mask_api_keys:
            return f"{self.config.api.api_key[:8]}..." if len(self.config.api.api_key) > 8 else "***"
