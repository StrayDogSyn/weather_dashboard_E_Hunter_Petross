"""
Unit tests for validation utilities and data validators.

Tests input validation, data sanitization, and validation utilities
used throughout the Weather Dashboard application.
"""
...
class TestTemperatureValidation(unittest.TestCase):
    ...
    def test_temperature_string_conversion(self):
        """Test temperature validation with string input."""
        self.assertTrue(validate_temperature_range("25.5", "C"))
        self.assertTrue(validate_temperature_range("77", "F"))
        
        # Invalid string conversion
        self.assertFalse(validate_temperature_range("not_a_number", "C"))
...
class TestCityNameValidation(unittest.TestCase):
    ...
    def test_city_name_edge_cases(self):
        ...
        # Names with numbers (valid in some cases)
        self.assertTrue(validate_city_name("Highway 401"))
...
class TestDateRangeValidation(unittest.TestCase):
    ...
    def test_valid_date_ranges(self):
        ...
        # Normal range
        self.assertTrue(validate_date_range(yesterday, tomorrow))
        
        # Same day
        self.assertTrue(validate_date_range(now, now))
    ...
    def test_invalid_date_ranges(self):
        ...
        # End before start
        self.assertFalse(validate_date_range(tomorrow, yesterday))
        
        # None values
        self.assertFalse(validate_date_range(None, tomorrow))
        self.assertFalse(validate_date_range(yesterday, None))


class TestWeatherDataValidator(unittest.TestCase):
    """Test cases for the WeatherDataValidator class."""
    ...
    def test_validate_weather_data_success(self):
        """Test successful weather data validation."""
        result = self.validator.validate_weather_data(self.valid_weather_data)
        self.assertTrue(result)
    
    def test_validate_weather_data_invalid_humidity(self):
        """Test weather data validation with invalid humidity."""
        invalid_data = self.valid_weather_data.copy()
        invalid_data['humidity'] = 150  # Invalid > 100
        
        with self.assertRaises(ValidationError):
            self.validator.validate_weather_data(invalid_data)
    
    def test_validate_weather_data_invalid_pressure(self):
        """Test weather data validation with invalid pressure."""
        invalid_data = self.valid_weather_data.copy()
        invalid_data['pressure'] = -10  # Invalid negative pressure
        
        with self.assertRaises(ValidationError):
            self.validator.validate_weather_data(invalid_data)
    
    def test_validate_weather_data_missing_fields(self):
        """Test weather data validation with missing required fields."""
        incomplete_data = {'temperature': 25.0}  # Missing other required fields
        
        with self.assertRaises(ValidationError):
            self.validator.validate_weather_data(incomplete_data)
...
class TestDataFormatters(unittest.TestCase):
    ...
    def test_wind_speed_formatting(self):
        ...
        self.assertEqual(format_wind_speed(25.75, 1), "25.8 km/h")
    
    def test_humidity_formatting(self):
        """Test humidity formatting."""
        self.assertEqual(format_humidity(65), "65%")
    ...
    def test_date_time_formatting(self):
        ...
        test_date = datetime(2025, 6, 27, 14, 30, 0)
        
        # Default format
        formatted = format_date_time(test_date)
        self.assertIn("2025", formatted)
        self.assertIn("Jun", formatted)
        self.assertIn("27", formatted)
        
        # Custom format
        custom_formatted = format_date_time(test_date, "%Y-%m-%d")
        self.assertEqual(custom_formatted, "2025-06-27")
    
    def test_city_name_formatting(self):
        """Test city name formatting."""
        # Title case formatting
        self.assertEqual(format_city_name("new york"), "New York")
        self.assertEqual(format_city_name("LOS ANGELES"), "Los Angeles")
        self.assertEqual(format_city_name("saN frAnCiScO"), "San Francisco")
        
        # Handle special cases
        self.assertEqual(format_city_name("mcdonald's"), "McDonald's")
        self.assertEqual(format_city_name("o'connor"), "O'Connor")
...
class TestValidationErrorHandling(unittest.TestCase):
    ...
    def test_validation_error_with_details(self):
        ...
        error = ValidationError("Invalid data", {"field": "temperature", "value": 150})
        
        self.assertIn("Invalid data", str(error))
...
if __name__ == '__main__':
    ...
    test_suite.addTest(loader.loadTestsFromTestCase(TestDateRangeValidation))
    test_suite.addTest(loader.loadTestsFromTestCase(TestWeatherDataValidator))
