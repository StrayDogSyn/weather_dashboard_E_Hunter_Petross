...
def test_poetry_service():
    ...
    try:
        ...
        from src.models.weather_models import (
            CurrentWeather, WeatherCondition, Temperature, TemperatureUnit, 
            Location, AtmosphericPressure, Wind
        )
        from datetime import datetime
        
        # Create a mock weather object
        location = Location(name="Test City", country="US", latitude=40.7128, longitude=-74.0060)
        ...
        print("=" * 50)
        print(f"AI Enhancement: {'Enabled' if poetry_service.ai_enabled else 'Disabled (template-based)'}")
        print()
